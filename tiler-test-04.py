...
Options:
  ...
  --frames INT       Number of frames (default 12).
  --tiles INT        Number of tiles to sample once and reuse across frames (default 5).
  --tile-w INT       Tile width in pixels (optional).
  --tile-h INT       Tile height in pixels (optional).
  --tile-size-percent F  Fraction of min(image_w,image_h) used when tile size not provided (default 0.20).
  --min-scale F      Minimum magnification (default 1.2).
  --max-scale F      Maximum magnification (default 1.6).
  --background COLOR Background color used when flattening tiles and for canvas (default "#ffffff").
  --duration INT     Frame duration in milliseconds (default 100).
  --loop INT         GIF loop count (0 infinite) (default 0).
  --seed INT         Optional base seed for reproducible tile sampling and frame permutations.
  --save-frames-dir DIR  Optional directory to save each generated frame as PNG.
