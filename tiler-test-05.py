...
This enhanced version adds two new features requested:
  ...
  2) animate subset option: choose a subset of tiles to animate across frames (their center and/or
     ...
     --animate-count N
     --animate-max-offset-frac F  (fraction of tile width/height to use as max translation)
     --animate-scale-delta-frac F (fractional +/- change to the base tile scale)
     Animation uses a smooth sinusoidal ease so looping looks continuous.
